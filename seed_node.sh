...
CHAINID="tok_2121-1"
MONIKER="second"
MAINNODE_RPC="https://rpc2.tokrpc.com"
MAINNODE_ID="fd930ec05a4120c9f80086351354127545f659f7@8.209.96.231:26656"
KEYRING="os"
CONFIG="$HOME/.tokd/config/config.toml"
APPCONFIG="$HOME/.tokd/config/app.toml"

# install chain binary file
make install

# Set moniker and chain-id for chain (Moniker can be anything, chain-id must be same mainnode)
tokd init $MONIKER --chain-id=$CHAINID

# Fetch genesis.json from genesis node
curl $MAINNODE_RPC/genesis? | jq ".result.genesis" > ~/.tokd/config/genesis.json

tokd validate-genesis

# set seed to main node's id manually
# sed -i 's/seeds = ""/seeds = "'$MAINNODE_ID'"/g' ~/.tokd/config/config.toml
...
tokd keys add $VALIDATOR --keyring-backend $KEYRING

# run node
#tokd start --rpc.laddr tcp://0.0.0.0:26657 --pruning=nothing