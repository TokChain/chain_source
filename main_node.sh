...
LOGLEVEL="info"
# Set dedicated home directory for the tokd instance
HOMEDIR="$HOME/.tokd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	tokd config keyring-backend $KEYRING --home "$HOMEDIR"
	tokd config chain-id $CHAINID --home "$HOMEDIR"

	# If keys exist they should be deleted
	for KEY in "${KEYS[@]}"; do
		tokd keys add $KEY --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR" 
	done

	# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
	tokd init $MONIKER -o --chain-id $CHAINID --home "$HOMEDIR"

	# Change parameter token denominations to tok
	jq '.app_state["staking"]["params"]["bond_denom"]="atok"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["crisis"]["constant_fee"]["denom"]="atok"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="atok"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["evm"]["params"]["evm_denom"]="atok"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["inflation"]["params"]["mint_denom"]="atok"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	claims_key=${KEYS[0]}
	# node_address=$(tokd keys show $claims_key --keyring-backend $KEYRING --home "$HOMEDIR" | grep "address" | cut -c12-)
	...
	sed -i 's/address = "127.0.0.1:8545"/address = "0.0.0.0:8545"/g' ~/.tokd/config/app.toml
	sed -i 's/ws-address = "127.0.0.1:8546"/ws-address = "0.0.0.0:8546"/g' ~/.tokd/config/app.toml
	sed -i 's/tracer = ""/tracer = "json"/g' ~/.tokd/config/app.toml
	...
	# sed -i 's/address = "tcp:\/\/0.0.0.0:1317"/address = "tcp:\/\/127.0.0.1:1317"/g' ~/.tokd/config/app.toml
	# sed -i 's/address = "0.0.0.0:9090"/address = "127.0.0.1:9090"/g' ~/.tokd/config/app.toml
	# sed -i 's/address = "0.0.0.0:9091"/address = "127.0.0.1:9091"/g' ~/.tokd/config/app.toml
	...
	sed -i '/\[api\]/,+3 s/enable = false/enable = true/' ~/.tokd/config/app.toml
	sed -i 's/swagger = false/swagger = true/g' ~/.tokd/config/app.toml
	sed -i 's/enabled-unsafe-cors = false/enabled-unsafe-cors = true/g'  ~/.tokd/config/app.toml
	sed -i 's/enable-indexer = false/enable-indexer = true/g' ~/.tokd/config/app.toml
	sed -i 's/api = "eth,net,web3"/api = "eth,txpool,personal,net,debug,web3,pubsub,trace"/g' ~/.tokd/config/app.toml
	sed -i 's/pruning = "default"/pruning = "nothing"/g' ~/.tokd/config/app.toml
	...
	# Collect genesis tx
	tokd collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	tokd validate-genesis --home "$HOMEDIR"
...
# tokd start --pruning=nothing "$TRACE" --gas-prices auto --gas-adjustment 1.3 --fees auto --rpc.laddr tcp://0.0.0.0:26657 --log_level $LOGLEVEL --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR"

# tokd tx staking create-validator --amount=1000000000000000000000atok --from=validator2 --pubkey=$(tokd tendermint show-validator) --moniker="validator2" --chain-id tok_2121-1 --commission-rate="0.1" --commission-max-rate="0.2" --commission-max-change-rate="0.05" --min-self-delegation="500000000" --keyring-backend=test --yes --broadcast-mode block