...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tok \
...
build-docker:
	...
	$(DOCKER) create --name tok -t -i ${DOCKER_IMAGE}:latest tok
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp tok:/usr/bin/tokd ./build/
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/tok:Z tok/node "./tokd testnet init-files --v 4 -o /tok --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\tokd:/evmos\Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node1\tokd:/evmos\Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node2\tokd:/evmos\Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node3\tokd:/evmos\Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
else
	@docker run --rm -v $(CURDIR)/build/node0/tokd:/evmos:Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node1/tokd:/evmos:Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node2/tokd:/evmos:Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node3/tokd:/evmos:Z evmos/node "./tokd tendermint unsafe-reset-all --home=/evmos"
endif
